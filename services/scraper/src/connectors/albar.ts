...
export const albarConnector = makePlaywrightConnector({
  name: 'albar',
  baseUrl: 'https://www.albar.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.albar.co.il/used-cars${qs ? '?' + qs : ''}`
  },
