...
export const toyota_selectConnector = makePlaywrightConnector({
  name: 'toyota_select',
  baseUrl: 'https://www.toyota-select.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.toyota-select.co.il/cars${qs ? '?' + qs : ''}`
  },
    ...
    if (nextData) {
      ...
      if (cars.length > 0) {
        return cars.slice(0, 40).map((v: any) => ({
          ...
          href:  v.url ?? v.link ?? `/toyota_select/${v.id}`,
        }))
      }
    }
    return page.$eval(
