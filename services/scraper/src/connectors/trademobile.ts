...
export const trademobileConnector = makePlaywrightConnector({
  name: 'trademobile',
  baseUrl: 'https://www.trademobile.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.trademobile.co.il/used-cars${qs ? '?' + qs : ''}`
  },
    ...
    if (nextData) {
      ...
      if (cars.length > 0) {
        return cars.slice(0, 40).map((v: any) => ({
          ...
          href:  v.url ?? v.link ?? `/trademobile/${v.id}`,
        }))
      }
    }
    return page.$eval(
