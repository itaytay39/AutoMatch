...
export const hertzConnector = makePlaywrightConnector({
  name: 'hertz',
  baseUrl: 'https://www.hertz.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.hertz.co.il/used-cars${qs ? '?' + qs : ''}`
  },
