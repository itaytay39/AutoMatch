...
export const shlomoConnector = makePlaywrightConnector({
  name: 'shlomo',
  baseUrl: 'https://www.shlomo.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.shlomo.co.il/used-cars${qs ? '?' + qs : ''}`
  },
