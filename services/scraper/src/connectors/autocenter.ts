...
export const autocenterConnector = makePlaywrightConnector({
  name: 'autocenter',
  baseUrl: 'https://www.autocenter.co.il',
  buildUrl: (c: SearchCriteria) => {
    ...
    const qs = q.toString()
    return `https://www.autocenter.co.il/cars${qs ? '?' + qs : ''}`
  },
    ...
    if (nextData) {
      ...
      if (cars.length > 0) {
        return cars.slice(0, 40).map((v: any) => ({
          ...
          href:  v.url ?? v.link ?? `/autocenter/${v.id}`,
        }))
      }
    }
